...
class HostViewController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        showCustomViewButton.enabled = false
    ...
    @IBAction func selectInstrumentOrEffect(sender: AnyObject?) {
        ...
        if (self.effectTable.selectedRow <= 0) {
            self.showCustomViewButton.enabled = false
        } else {
            self.showCustomViewButton.enabled = true
        }
    ...
    @IBAction func openViewAction(sender: AnyObject?) {
        if (isDisplayingCustomView) {
            if (auView != nil) {
                closeAUView()
                return
            ...
            playEngine.testAudioUnit?.requestViewControllerWithCompletionHandler {
                ...
                strongSelf.verticalLine.hidden = false
                ...
                NSNotificationCenter.defaultCenter().addObserver(strongSelf, selector: #selector(HostViewController.auViewSizeChanged(_:)), name:
                    NSViewFrameDidChangeNotification, object: nil)
    ...
    func auViewSizeChanged(notification : NSNotification) {
        if (notification.object! === auView) {            
            self.horizontalViewSizeConstraint.constant = notification.object!.frame.size.width
            
            if (notification.object!.frame.size.height > self.kAUViewSizeDefaultHeight) {
                self.verticalViewSizeConstraint.constant = notification.object!.frame.size.height
            }
        }
    }
    ...
    func tableViewSelectionDidChange(aNotification: NSNotification) {
        ...
        if tableView === effectTable {
            ...
            if row > 0 {
                ...
                showCustomViewButton.enabled = true
            } else {
                component = nil
                showCustomViewButton.enabled = false
            }
