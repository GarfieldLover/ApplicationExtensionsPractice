...
class HostViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if tableView === audioUnitTableView {
            if indexPath.row > 0 && indexPath.row <= playEngine.availableAudioUnits.count {
                let component = playEngine.availableAudioUnits[indexPath.row - 1]
    ...
    @IBAction func selectInstrumentOrEffect(sender: AnyObject?) {
        ...
        audioUnitTableView.selectRowAtIndexPath(NSIndexPath(forRow: 0, inSection: 0), animated: false, scrollPosition: UITableViewScrollPosition.Top)
	...
	@IBAction func toggleView(sender: AnyObject?) {
        ...
        */
        playEngine.testAudioUnit?.requestViewControllerWithCompletionHandler { [weak self] viewController in
