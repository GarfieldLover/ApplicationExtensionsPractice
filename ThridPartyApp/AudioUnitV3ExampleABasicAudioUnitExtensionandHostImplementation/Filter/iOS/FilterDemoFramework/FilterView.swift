...
class FilterView: UIView {
	 ...
	 */
    func valueAtGridIndex(index: Float) -> Float {   
        return FilterView.defaultMinHertz * powf(Float(logBase), index)
    }
    
    func logValueForNumber(number: Float, base: Float) -> Float {
	 ...
	 */
    func setMagnitudes(magnitudeData: [Double]) {
        ...
        CGPathMoveToPoint(bezierPath, nil, leftMargin, graphLayer.frame.height + bottomMargin)
        ...
        CGPathAddLineToPoint(bezierPath, nil, location, graphLayer.frame.origin.y + graphLayer.frame.height + bottomMargin)
    ...
    /*
		...
		the dB value.
	 */
    func locationForDBValue(value: Float) -> CGFloat {
    ...
    /*
		...
		the frequency value.
     */
    func locationForFrequencyValue(value: Float) -> CGFloat {
	...
	/*
		...
		the graph.
	 */
    func dbValueForLocation(location: CGFloat) -> Float {        
        let step = graphLayer.frame.height / CGFloat(defaultGain * 2)
    ...
    /*
		...
		axis of the graph.
	 */
    func frequencyValueForLocation(location: CGFloat) -> Float {
        let pixelIncrement = graphLayer.frame.width / CGFloat(gridLineCount)
    ...
    /*
		...
		input value.
	 */
    func stringForValue(value: Float) -> String {
       var temp = value
    ...
    override func awakeFromNib() {
        ...
        graphLayer.borderColor = UIColor.darkGrayColor().CGColor
    ...
    func createDBLabelsAndLines() {
        ...
        for index in -numDBLines ... numDBLines {
            ...
            if index >= -numDBLines && index <= numDBLines {
                ...
                labelLayer.font = UIFont.systemFontOfSize(14).fontName
    ...
    func createControlPoint() {
        ...
        let controlColor = touchDown ? tintColor.CGColor: UIColor.darkGrayColor().CGColor
     ...
     */
    func updateControls (refreshColor refreshColor: Bool) {
        let color = touchDown ? tintColor.CGColor: UIColor.darkGrayColor().CGColor
        ...
        for layer in controls {
            switch layer.name! {
                case "point":
                    layer.frame = CGRectIntegral(CGRect(x: editPoint.x - 3, y: editPoint.y-3, width: 7, height: 7))
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        var pointOfTouch = touches.first?.locationInView(self)
        
        pointOfTouch = CGPoint(x: pointOfTouch!.x, y: pointOfTouch!.y + bottomMargin)
        
        if graphLayer.containsPoint(pointOfTouch!) {
            touchDown = true
    ...
    func processTouch(touchPoint: CGPoint) {
        if touchPoint.x < 0 {
            editPoint.x = 0
        }
