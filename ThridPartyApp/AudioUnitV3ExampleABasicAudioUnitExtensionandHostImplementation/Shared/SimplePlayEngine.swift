...
public class SimplePlayEngine : NSObject {
	...
	private var componentType: UInt32
	{
		willSet {
            ...
            if testUnitNode != nil {
                ...
                engine.detachNode(testUnitNode!)
	...
	private let availableAudioUnitsAccessQueue = dispatch_queue_create("SimplePlayEngine.availableAudioUnitsAccessQueue", DISPATCH_QUEUE_SERIAL)
    ...
    */
    var availableAudioUnits: [AVAudioUnitComponent] {
        get {
            ...
            dispatch_sync(availableAudioUnitsAccessQueue) {
	...
	public init(componentType inComponentType: UInt32, componentsFoundCallback inComponentsFoundCallback: (Void -> Void)? = nil) {
        ...
        */
		NSNotificationCenter.defaultCenter().addObserverForName(String(kAudioComponentInstanceInvalidationNotification), object: nil, queue: nil) { [weak self] notification in
			...
			*/
			if let crashedAU = notification.object as? AUAudioUnit where strongSelf.testAudioUnit === crashedAU {
    ...
    */
	private func updateAudioUnitList() {
		dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0)) {
			...
			*/
			...
			self.availableAudioUnits = AVAudioUnitComponentManager.sharedAudioUnitComponentManager().componentsMatchingDescription(componentDescription)
	...
	public func selectPresetIndex(presetIndex: Int) {
	...
	public func selectAudioUnitComponent(component: AVAudioUnitComponent?, completionHandler: Void -> Void) {
        selectAudioUnitWithComponentDescription(component?.audioComponentDescription, completionHandler: completionHandler)
	}
	
    public func selectAudioUnitWithComponentDescription2(componentDescription: AudioComponentDescription, completionHandler: (Void -> Void)) {
	...
	*/
    public func selectAudioUnitWithComponentDescription(componentDescription: AudioComponentDescription?, completionHandler: (Void -> Void)) {
...
internal class InstrumentPlayer : NSObject {
    ...
    private func scheduleInstrumentLoop() {
        ...
        let cbytes = UnsafeMutablePointer<UInt8>.alloc(3)
